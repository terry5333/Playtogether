...
io.on('connection', (socket) => {
    socket.on('join_room', (data) => {
        ...
        if (!rooms[roomId]) rooms[roomId] = { gameType, host: socket.id, players: [], gameStarted: false, turnIdx: 0, roles: {} };
    ...
    socket.on('bingo_win', (data) => io.to(data.roomId).emit('game_over', { msg: `恭喜 ${data.name} 獲勝！`, subMsg: "BINGO! 達成連線" }));
